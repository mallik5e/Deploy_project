...
const InvoiceTemplate1 = () => {
  ...
  //console.log("user: ",user)
      ...
      const invoiceData = {
        ...
        items: transaction?.services || transaction?.addOns || [
...

{
        ...
        (
          ...
          <input type="text" className="text-center" placeholder="Your Company Website"  name="website"  onChange={handleChange}  />
          </div>
          </div>
        )
        : (
          <div className="flex mb-5">
        <img src={user.logo} alt="No Logo" className="w-32 h-16 rounded"/>
      <div className="mx-4">
      <p className="md:text-2xl font-semibold">{invoiceData.seller.name}</p>
      <p className="text-[14px]">{invoiceData.seller.website}</p>
      </div>
      </div>
        )
  }
   {
      ...
      <div className="grid grid-cols-2 gap-4   border-b pb-4 mb-4">
        <div className="">
          ...
          <hr className="w-2/3 border-t border-blue-400" />
      ...
      <table className="w-full border-collapse border border-gray-300 mb-4">
        <thead>
          <tr className="bg-blue-500 text-white">
            <th className="border-t p-2">Item Description</th>
      ...
      <div className="text-right ">
        ...
        <p className="text-xl text-blue-500 font-bold px-3">Total: ${invoiceData.total}.00</p>
      </div>
      ...
      <hr className=" w-1/3 border-t border-gray-400 ml-auto" />
        <p className="md:mx-8">Company Signature</p>
      </div>
      ...
      <svg
    className="absolute bottom-0 right-0 w-45 h-18"
    viewBox="0 0 120 60"
    preserveAspectRatio="none"
  >
    
    <path d="M 0 70 Q 60 -25 130 38" fill="rgb(59, 130, 246)" />
    <path d="M 0 110 Q 50 -20 140 50" fill="black" />
  </svg>
...
<button title="Edit Template" className="absolute hidden 2xl:block bg-green-200 p-2 rounded-lg top-20 right-80" onClick={()=>setIsEdit(true)}><FiEdit size={32}/></button>
  </div>
  )}
    </div>
    )
