  ...
  <div className="flex justify-center my-3">
    <div className="relative w-full max-w-2xl bg-white shadow-md rounded-lg border border-gray-300 p-5">
     ...
     <div>
     {
              ...
              <input type="text" className="text-xl font-semibold border border-gray-300 rounded-sm" placeholder=" Your Company name" name="name" onChange={handleChange}/>
              <input type="text" className="border border-gray-300 rounded-sm" placeholder=" Company website" name="website" onChange={handleChange}/>
              <input type="text" className=" border border-gray-300 rounded-sm" placeholder=" Company address" name="address" onChange={handleChange}/>
              <input type="text" className=" border border-gray-300 rounded-sm" placeholder=" Company contact" name="contact" onChange={handleChange}/>
              </div>
      ...
      <div className="px-5 my-auto mr-6 md:mr-18">
...
<button   title="Close" className="absolute top-0 right-0 text-red-600 font-bold bg-red-200 shadow-2xl transition"  onClick={handleCloseOverlay}>
  <RxCross2 size={45}/>
      ...
      <div className="grid grid-cols-2 gap-4 border-b py-2 mb-4">
        <div className="">
  ...
  <tbody className="">
    {invoiceData.items.map((item, index) => (
      <tr
        key={index}
       className="bg-gray-100 rounded-lg shadow-sm mb-2 justify-between items-center"
      >
        <td className="p-3 text-gray-800">{item.name}</td>
        <td className="p-3 text-center text-gray-800">{item.quantity}</td>
        <td className="p-3 text-center text-gray-800">${item.price}</td>
    ...
    {invoiceData.addOns.map((addOn, index) => (
            <tr key={index} className="bg-gray-100 rounded-lg shadow-sm mb-2 justify-between items-center">
              <td className="p-2">{addOn.name}</td>
              <td className="p-2 text-center">1</td>
              <td className="p-2 text-center">${addOn.price}</td>
              <td className="p-2 text-center">${(1 * addOn.price)}</td>
            </tr>
          ))}
  </tbody>
</table>
      ...
      <div className="text-right mt-3">
        <p className="px-3"><span className="font-semibold">Subtotal:</span> ${invoiceData.subtotal}.00</p>
        <p className="px-3"><span className="font-semibold">Discount: </span> ${invoiceData.Discount}.00</p>
        <p className="px-3"><span className="font-semibold">Total Tax:</span> ${invoiceData.tax}.00</p>
     ...
     <div>
      ...
      <div>
        <p className=""><strong>Terms and Instructions:</strong> </p>
